...
1. Файл sqldelight/.../**user.sq** — На языке **SQL** (диалект SQLite) опишите команды создания, наполнения и редактирования таблицы пользователей для локальной базы данных;
2. Файл **Server.kt** — На языке **Kotlin** с использованием фреймворка **Ktor-сервер** опишите эндпоинты серверного API, обеспечивающие все необходимые для выполнения задания функции;
3. Файл **Client.kt** — На языке **Kotlin** с использованием фреймворка **Ktor-клиент** опишите клиентские функции доступа к серверному API;
4. Файл **Application.kt** — На языке **Kotlin** с использованием фреймворка **Compose** опишите интерфейс пользователя в соответствии с заданием;
5. Файл **Editor.kt** — В отдельный файл может быть вынесен экран редактирования пользователей.
